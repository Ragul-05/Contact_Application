...
export default function CreateContact({ onContactAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onContactAdded(res.data); // Notify parent to refresh list
    } catch (error) {
      setStatus(`Error: ${error.response?.data?.message || 'Failed to create contact'}`);
    }
  };

  return (
    <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full mx-auto mt-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Add New Contact</h2>
